/*
 * File: Copter_Plane_h2g_4_and_1.c
 *
...
#include "Copter_Plane_h2g_4_and_1.h"
#define NumBitsPerChar                 8U

const BUS_TASK_PATH_OutParam Copter_Plane_h2g_4_and_1_rtZBUS_TASK_PATH_OutParam =
{
  0.0,                                 /* currentPointNum */
  0.0,                                 /* prePointNum */
  0.0,                                 /* validPathNum */
...
static void attitude_controller_run_quat(const real_T attitude_target_ang_vel[3],
  DW *rtDW);
static void input_euler_angle_roll_pitch_eu(real_T euler_roll_angle_cd, real_T
...
static void update_speed(real_T load_factor, real_T *TAS_dem, real_T *TASmax,
  real_T *TASmin, DW *rtDW);
...
static real_T get_bearing_to(const real_T loc2[2], const real_T b_loc[2], DW
  *rtDW);
static real_T prevent_indecision(real_T Nu, const real_T *target_bearing_cd, DW *
  rtDW);
...
static void auto_mode_4a1(real_T prev_WP[2], real_T next_WP[2], DW *rtDW);
...
static void auto_mode_sl_4a1(real_T prev_WP[2], real_T next_WP[2], DW *rtDW);
...
static void auto_mode(real_T prev_WP[2], real_T next_WP[2], DW *rtDW);
static void copter_run_posfree(DW *rtDW);
static void auto_mode_sl(real_T prev_WP[2], real_T next_WP[2], DW *rtDW);
  ...
  real_T filter_cutoff, DW *rtDW)
{
  ...
  if ((apply_angle_boost != 0.0) && (rtDW->angle_boost_enabled != 0.0)) {
  ...
  DW *rtDW)
{
  ...
  quatmultiply(q, tmp_0, yaw_vec_correction_quat);
  quatmultiply(yaw_vec_correction_quat, attitude_vehicle_quat,
  ...
  thrust_vector_length = att_to_thrust_vec[0];
  att_diff_angle_y = att_to_thrust_vec[1];
  b_l = sqrt((yaw_vec_correction_quat[1] * yaw_vec_correction_quat[1] +
              yaw_vec_correction_quat[2] * yaw_vec_correction_quat[2]) +
             yaw_vec_correction_quat[3] * yaw_vec_correction_quat[3]);
  att_to_thrust_vec[2] = yaw_vec_correction_quat[3];
  if (b_l != 0.0) {
    att_to_thrust_vec[2] = yaw_vec_correction_quat[3] / b_l;
    b_l = wrap_PI(2.0 * rt_atan2d_snf(b_l, yaw_vec_correction_quat[0]));
    att_to_thrust_vec[2] *= b_l;
  }

  b_l = att_to_thrust_vec[2];
  if (rtDW->p_angle_yaw != 0.0) {
    ...
    if (fabs(att_to_thrust_vec[2]) > high) {
      ...
      if (b_theta == 0.0) {
        ...
        att_to_thrust_vec[2] = b_l / b_theta;
  ...
  att_to_thrust_vec[0] = thrust_vector_length;
  att_to_thrust_vec[1] = att_diff_angle_y;
  att_to_thrust_vec[2] = b_l;
  if (rtDW->use_sqrt_controller != 0.0) {
    thrust_vector_length = rtDW->accel_roll_max * 0.01 / 2.0;
    att_diff_angle_y = thrust_vector_length;
    if (thrust_vector_length < rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLL_j) {
      att_diff_angle_y = rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLL_j;
    }

    if (thrust_vector_length > rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLLER) {
      att_diff_angle_y = rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLLER;
    }

    rtDW->rate_target_ang_vel[0] = sqrt_controller(att_to_thrust_vec[0],
      rtDW->p_angle_roll, att_diff_angle_y, rtDW->dt);
    att_diff_angle_y = rtDW->accel_pitch_max * 0.01 / 2.0;
    b_l = att_diff_angle_y;
    if (att_diff_angle_y < rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLL_j) {
      b_l = rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLL_j;
    }

    if (att_diff_angle_y > rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLLER) {
      b_l = rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLLER;
    }

    rtDW->rate_target_ang_vel[1] = sqrt_controller(att_to_thrust_vec[1],
      rtDW->p_angle_pitch, b_l, rtDW->dt);
    b_l = rtDW->accel_yaw_max * 0.01 / 2.0;
    b_theta = b_l;
    if (b_l < rtDW->AC_ATTITUDE_ACCEL_Y_CONTROLLE_g) {
      b_theta = rtDW->AC_ATTITUDE_ACCEL_Y_CONTROLLE_g;
    }

    if (b_l > rtDW->AC_ATTITUDE_ACCEL_Y_CONTROLLER_) {
      b_theta = rtDW->AC_ATTITUDE_ACCEL_Y_CONTROLLER_;
    }

    rtDW->rate_target_ang_vel[2] = sqrt_controller(att_to_thrust_vec[2],
      rtDW->p_angle_yaw, b_theta, rtDW->dt);
  } else {
    rtDW->rate_target_ang_vel[0] = rtDW->p_angle_roll * thrust_vector_length;
    rtDW->rate_target_ang_vel[1] = rtDW->p_angle_pitch * att_diff_angle_y;
    rtDW->rate_target_ang_vel[2] = rtDW->p_angle_yaw * b_l;
  }

  thrust_vector_length = att_to_thrust_vec[1];
  if (att_to_thrust_vec[1] < -0.78539816339744828) {
    thrust_vector_length = -0.78539816339744828;
  }
  ...
  rtDW->rate_target_ang_vel[0] += thrust_vector_length * rtDW->gyro_z;
  thrust_vector_length = att_to_thrust_vec[0];
  if (att_to_thrust_vec[0] < -0.78539816339744828) {
    thrust_vector_length = -0.78539816339744828;
  }
  ...
  rtDW->rate_target_ang_vel[1] -= thrust_vector_length * rtDW->gyro_z;
  for (i = 0; i < 3; i++) {
  ...
  q[2] = -thrust_vec_correction_quat[2];
  q[3] = -thrust_vec_correction_quat[3];
  attitude_vehicle_quat[0] = 0.0;
  attitude_vehicle_quat[1] = attitude_target_ang_vel[0];
  attitude_vehicle_quat[2] = attitude_target_ang_vel[1];
  attitude_vehicle_quat[3] = attitude_target_ang_vel[2];
  quatmultiply(q, attitude_vehicle_quat, tmp_0);
  quatmultiply(tmp_0, thrust_vec_correction_quat, yaw_vec_correction_quat);
  if (feedforward_scalar > rtDW->AC_ATTITUDE_THRUST_ERROR_ANGLE * 2.0) {
    rtDW->rate_target_ang_vel[2] = rtDW->gyro_z;
  } else if (feedforward_scalar > rtDW->AC_ATTITUDE_THRUST_ERROR_ANGLE) {
    feedforward_scalar = 1.0 - (feedforward_scalar -
  ...
  if (rtDW->rate_bf_ff_enabled != 0.0) {
    att_to_thrust_vec[0] = attitude_target_ang_vel[0] * rtDW->dt;
    att_to_thrust_vec[1] = attitude_target_ang_vel[1] * rtDW->dt;
    att_to_thrust_vec[2] = attitude_target_ang_vel[2] * rtDW->dt;
  ...
  euler_pitch_angle_cd, real_T euler_yaw_rate_cds, DW *rtDW)
{
  ...
  real_T attitude_target_euler_angle[3];
  real_T b_y_tmp;
  real_T c_y_tmp;
  real_T tmp[3];
  ...
  attitude_target_euler_angle[0] = rt_atan2d_snf((rtDW->attitude_target_quat[0] *
    rtDW->attitude_target_quat[1] + rtDW->attitude_target_quat[2] *
    rtDW->attitude_target_quat[3]) * 2.0, 1.0 - (rtDW->attitude_target_quat[1] *
    rtDW->attitude_target_quat[1] + attitude_target_euler_angle_tmp) * 2.0);
  attitude_target_euler_angle[1] = asin((rtDW->attitude_target_quat[0] *
    rtDW->attitude_target_quat[2] - rtDW->attitude_target_quat[3] *
    rtDW->attitude_target_quat[1]) * 2.0);
  attitude_target_euler_angle[2] = rt_atan2d_snf((rtDW->attitude_target_quat[0] *
    rtDW->attitude_target_quat[3] + rtDW->attitude_target_quat[1] *
    rtDW->attitude_target_quat[2]) * 2.0, 1.0 - (attitude_target_euler_angle_tmp
    + rtDW->attitude_target_quat[3] * rtDW->attitude_target_quat[3]) * 2.0);
  if (rtDW->rate_bf_ff_enabled != 0.0) {
    ...
    attitude_target_euler_angle_tmp = sin(attitude_target_euler_angle[0]);
    sin_phi = fabs(attitude_target_euler_angle_tmp);
    if (sin_phi < 0.1) {
      sin_phi = 0.1;
    }

    b_y_tmp = cos(attitude_target_euler_angle[0]);
    cos_phi = fabs(b_y_tmp);
    if (cos_phi < 0.1) {
      cos_phi = 0.1;
    }

    c_y_tmp = sin(attitude_target_euler_angle[1]);
    ...
    euler_roll_angle = sqrt_controller(wrap_PI(euler_roll_angle -
      attitude_target_euler_angle[0]), cos_phi, euler_accel_idx_0, rtDW->dt);
    ...
    euler_pitch_angle = sqrt_controller(wrap_PI(euler_pitch_angle -
      attitude_target_euler_angle[1]), cos_phi, euler_accel_idx_1, rtDW->dt);
    ...
    euler_pitch_angle = cos(attitude_target_euler_angle[1]);
    tmp[0] = rtDW->attitude_target_euler_rate[0] - c_y_tmp *
      rtDW->attitude_target_euler_rate[2];
    tmp[1] = attitude_target_euler_angle_tmp * euler_pitch_angle *
      rtDW->attitude_target_euler_rate[2] + b_y_tmp *
      rtDW->attitude_target_euler_rate[1];
    tmp[2] = euler_pitch_angle * b_y_tmp * rtDW->attitude_target_euler_rate[2] +
      -attitude_target_euler_angle_tmp * rtDW->attitude_target_euler_rate[1];
    ...
    rtDW->attitude_target_euler_rate[0] = (euler_roll_angle *
      attitude_target_euler_angle_tmp * attitude_target_euler_angle[1] +
      attitude_target_euler_angle[0]) + euler_roll_angle * b_y_tmp *
      attitude_target_euler_angle[2];
    rtDW->attitude_target_euler_rate[1] = b_y_tmp * attitude_target_euler_angle
      [1] - attitude_target_euler_angle_tmp * attitude_target_euler_angle[2];
    rtDW->attitude_target_euler_rate[2] = attitude_target_euler_angle_tmp /
      euler_pitch_angle * attitude_target_euler_angle[1] + b_y_tmp /
      euler_pitch_angle * attitude_target_euler_angle[2];
  } else {
    attitude_target_euler_angle[0] = euler_roll_angle;
    attitude_target_euler_angle[1] = euler_pitch_angle;
    attitude_target_euler_angle[2] += euler_yaw_rate * rtDW->dt;
    from_euler(attitude_target_euler_angle, rtDW->attitude_target_quat);
    rtDW->attitude_target_euler_rate[0] = 0.0;
    attitude_target_euler_angle[0] = 0.0;
    rtDW->attitude_target_euler_rate[1] = 0.0;
    attitude_target_euler_angle[1] = 0.0;
    rtDW->attitude_target_euler_rate[2] = 0.0;
    attitude_target_euler_angle[2] = 0.0;
  }

  attitude_controller_run_quat(attitude_target_euler_angle, rtDW);
}
...
static void update_z_controller(DW *rtDW)
{
  ...
  alpha = ((rtDW->POSCONTROL_ACC_Z_P * rtDW->pid_accel_z_input + i) +
           rtDW->POSCONTROL_ACC_Z_D * alpha) * 0.001 + rtDW->throttle_hover;
  i = alpha;
...
static void stabilize(DW *rtDW)
{
  ...
  scale_max = (scale_min - rtDW->gyro_x * rtDW->HD) * speed_scaler;
  if ((!(rtDW->disable_integrator_roll != 0.0)) && (ki_rate_tmp > 0.0)) {
    if ((rtDW->dt > 0.0) && (rtDW->aspeed > rtDW->airspeed_min)) {
      ki_rate = scale_max * ki_rate_tmp * rtDW->dt * speed_scaler;
  ...
  rtDW->last_out_roll = (fmax((rtDW->gains_P_roll - ki_rate_tmp) *
    rtDW->gains_tau_roll - rtDW->gains_D_roll, 0.0) / rtDW->EAS2TAS * scale_min *
    speed_scaler + rtDW->gains_FF_roll / rtDW->EAS2TAS * scale_min *
    speed_scaler) + scale_max * rtDW->gains_D_roll * speed_scaler;
  ...
  k_I = rtDW->pitch * rtDW->HD * 100.0;
  if (fabs(k_I) > 7000.0) {
    ki_rate = 0.0;
  } else {
    ki_rate = fabs(rtDW->GRAVITY_MSS / fmax(rtDW->aspeed * rtDW->EAS2TAS,
  ...
  scale_max = ((rtDW->k_throttle * rtDW->kff_throttle_to_pitch +
                rtDW->nav_pitch_cd) - k_I) * 0.01 / rtDW->gains_tau_pitch;
  if (inverted == 0) {
  ...
  scale_max += ki_rate;
  ki_rate = (scale_max - rtDW->gyro_y * rtDW->HD) * speed_scaler;
  ...
  rtDW->last_out_pitch = fmax((rtDW->gains_P_pitch - rtDW->gains_I_pitch *
    rtDW->gains_tau_pitch) * rtDW->gains_tau_pitch - rtDW->gains_D_pitch, 0.0) /
    rtDW->EAS2TAS * scale_max * speed_scaler + (rtDW->gains_FF_pitch /
    rtDW->EAS2TAS * scale_max * speed_scaler + ki_rate * rtDW->gains_D_pitch *
    speed_scaler);
  ...
  ki_rate = (rtDW->gyro_z - rtDW->GRAVITY_MSS / fmax(rtDW->aspeed *
              rtDW->EAS2TAS, scale_max * rtDW->EAS2TAS) * sin(ki_rate) *
             rtDW->K_FF_yaw) * rtDW->HD;
  scale_min = ((1.0 - 0.2 * rtDW->dt) * rtDW->last_rate_hp_out_yaw + ki_rate) -
    rtDW->last_rate_hp_in_yaw;
  rtDW->last_rate_hp_out_yaw = scale_min;
  rtDW->last_rate_hp_in_yaw = ki_rate;
  ki_rate = (rtDW->K_A_yaw * rtDW->accel_y + scale_min) * -rtDW->K_I_yaw;
  if ((!(rtDW->disable_integrator_yaw != 0.0)) && (rtDW->K_D_yaw > 0.0)) {
    if (rtDW->aspeed > scale_max) {
      if (rtDW->last_out_yaw < -45.0) {
        rtDW->integrator_yaw += fmax(ki_rate * rtDW->dt, 0.0);
      } else if (rtDW->last_out_yaw > 45.0) {
        rtDW->integrator_yaw += fmin(ki_rate * rtDW->dt, 0.0);
      } else {
        rtDW->integrator_yaw += ki_rate * rtDW->dt;
      }
    }
  } else {
  ...
  if (rtDW->K_D_yaw < 0.0001) {
    ...
    rtDW->last_out_yaw = rtDW->K_D_yaw * rtDW->integrator_yaw * speed_scaler *
      speed_scaler + rtDW->K_D_yaw * -scale_min * speed_scaler * speed_scaler;
  ...
  real_T *TASmin, DW *rtDW)
{
  ...
  *TAS_dem = rtDW->EAS_dem * rtDW->EAS2TAS;
  *TASmax = rtDW->airspeed_max * rtDW->EAS2TAS;
  *TASmin = rtDW->airspeed_min * rtDW->EAS2TAS;
  *TASmin *= load_factor;
  if (*TASmax < *TASmin) {
    *TASmax = *TASmin;
  }
  ...
  real_T load_factor, DW *rtDW)
{
  ...
  real_T TASmax;
  real_T TASmin;
  real_T SPE_err_max_tmp;
  rtDW->hgt_dem = b_hgt_dem_cm * 0.01;
  rtDW->EAS_dem = b_EAS_dem_cm * 0.01;
  update_speed(load_factor, &TAS_dem, &TASmax, &TASmin, rtDW);
  THRmaxf = rtDW->throttle_max * 0.01;
  THRminf = rtDW->throttle_min * 0.01;
  if (rtDW->pitch_max <= 0.0) {
    PITCHmaxf = rtDW->pitch_limit_max_cd * 0.01;
  } else {
    PITCHmaxf = fmin(rtDW->pitch_max, rtDW->pitch_limit_max_cd * 0.01);
  }

  if (rtDW->pitch_min >= 0.0) {
    PITCHminf = rtDW->pitch_limit_min_cd * 0.01;
  } else {
    PITCHminf = fmax(rtDW->pitch_min, rtDW->pitch_limit_min_cd * 0.01);
  }

  if (rtDW->pitch_max_limit < 90.0) {
    velRateMin = PITCHmaxf;
    if (PITCHmaxf < -90.0) {
      PITCHmaxf = -90.0;
    }

    if (velRateMin > rtDW->pitch_max_limit) {
      PITCHmaxf = rtDW->pitch_max_limit;
    }

    velRateMin = PITCHminf;
    if (PITCHminf < -rtDW->pitch_max_limit) {
      PITCHminf = -rtDW->pitch_max_limit;
    }

    if (velRateMin > PITCHmaxf) {
      PITCHminf = PITCHmaxf;
    }
  ...
  PITCHmaxf = PITCHmaxf / 180.0 * 3.1415926535897931;
  PITCHminf = PITCHminf / 180.0 * 3.1415926535897931;
  STEdot_max = rtDW->maxClimbRate * rtDW->GRAVITY_MSS;
  STEdot_min = -rtDW->minSinkRate * rtDW->GRAVITY_MSS;
  velRateMin = TAS_dem;
  if (TAS_dem < TASmin) {
    TAS_dem = TASmin;
  }

  if (velRateMin > TASmax) {
    TAS_dem = TASmax;
  }

  velRateMax = 0.5 * STEdot_max / rtDW->TAS_state;
  velRateMin = 0.5 * STEdot_min / rtDW->TAS_state;
  SPEdot_dem = TAS_dem - rtDW->TAS_dem_adj;
  SKEdot_dem = velRateMax * rtDW->dt;
  if (SPEdot_dem > SKEdot_dem) {
    rtDW->TAS_dem_adj += SKEdot_dem;
    velRateMin = velRateMax;
  } else if (SPEdot_dem < velRateMin * rtDW->dt) {
    rtDW->TAS_dem_adj += velRateMin * rtDW->dt;
  } else {
    velRateMin = (TAS_dem - rtDW->TAS_dem_adj) / rtDW->dt;
    rtDW->TAS_dem_adj = TAS_dem;
  }

  TAS_dem = rtDW->TAS_dem_adj;
  if (rtDW->TAS_dem_adj < TASmin) {
    rtDW->TAS_dem_adj = TASmin;
  }

  if (TAS_dem > TASmax) {
    rtDW->TAS_dem_adj = TASmax;
  }
  ...
  rtDW->hgt_dem_in_old = rtDW->hgt_dem;
  SPEdot_dem = rtDW->hgt_dem - rtDW->hgt_dem_prev;
  SKEdot_dem = rtDW->maxClimbRate * rtDW->dt;
  if (SPEdot_dem > SKEdot_dem) {
    rtDW->hgt_dem = SKEdot_dem + rtDW->hgt_dem_prev;
  } else {
    if (SPEdot_dem < -rtDW->maxSinkRate * rtDW->dt) {
      rtDW->hgt_dem = rtDW->hgt_dem_prev - rtDW->maxSinkRate * rtDW->dt;
    }
  }

  rtDW->hgt_dem_prev = rtDW->hgt_dem;
  velRateMax = 0.05 * rtDW->hgt_dem + 0.95 * rtDW->hgt_dem_adj_last;
  TAS_dem = (velRateMax - rtDW->hgt_dem_adj_last) / rtDW->dt;
  rtDW->hgt_dem_adj_last = velRateMax;
  SPE_dem = velRateMax * rtDW->GRAVITY_MSS;
  velRateMax = 0.5 * rtDW->TAS_dem_adj * rtDW->TAS_dem_adj;
  SPEdot_dem = TAS_dem * rtDW->GRAVITY_MSS;
  SKEdot_dem = rtDW->TAS_state * velRateMin;
  SKE_est = 0.5 * rtDW->TAS_state * rtDW->TAS_state;
  velRateMin = rtDW->climb_rate * rtDW->GRAVITY_MSS;
  TAS_dem = rtDW->TAS_state * rtDW->vel_dot;
  SPE_err_max_tmp = 0.5 * TASmax * TASmax - velRateMax;
  TASmin = 0.5 * TASmin * TASmin - velRateMax;
  SPE_dem -= rtDW->height * rtDW->GRAVITY_MSS;
  TASmax = SPE_dem;
  if (SPE_dem < TASmin) {
    TASmax = TASmin;
  }

  if (SPE_dem > SPE_err_max_tmp) {
    TASmax = SPE_err_max_tmp;
  }

  SPE_err_max = (TASmax + velRateMax) - SKE_est;
  SPE_err_min = SPEdot_dem + SKEdot_dem;
  STEdot_dem = SPE_err_min;
  if (SPE_err_min < STEdot_min) {
    STEdot_dem = STEdot_min;
  }

  if (SPE_err_min > STEdot_max) {
    STEdot_dem = STEdot_max;
  }

  TASmax = ((STEdot_dem - velRateMin) - TAS_dem) * 0.2 + 0.8 *
    rtDW->STEdotErrLast;
  rtDW->STEdotErrLast = TASmax;
  STEdot_max -= STEdot_min;
  STEdot_min = THRmaxf - THRminf;
  SPE_err_min = 1.0 / (STEdot_max * rtDW->timeConstant / STEdot_min);
  ...
  STEdot_dem += (1.0 / b_amto - 1.0) * rtDW->rollComp;
  rtDW->throttle_dem = (STEdot_dem / STEdot_max * STEdot_min *
                        rtDW->p_ff_throttle + rtDW->throttle_cruise * 0.01) +
    (TASmax * rtDW->thrDamp + SPE_err_max) * SPE_err_min;
  STEdot_max = rtDW->throttle_dem;
  if (rtDW->throttle_dem < THRminf) {
    rtDW->throttle_dem = THRminf;
  }

  if (STEdot_max > THRmaxf) {
    rtDW->throttle_dem = THRmaxf;
  }
  ...
  if (rtDW->throttle_slewrate != 0.0) {
    TASmax = (THRmaxf - STEdot_max) * rtDW->dt * rtDW->throttle_slewrate * 0.01;
    STEdot_min = rtDW->throttle_dem;
    STEdot_dem = rtDW->last_throttle_dem - TASmax;
    TASmax += rtDW->last_throttle_dem;
    if (rtDW->throttle_dem < STEdot_dem) {
      rtDW->throttle_dem = STEdot_dem;
    }

    if (STEdot_min > TASmax) {
      rtDW->throttle_dem = TASmax;
    }

    rtDW->last_throttle_dem = rtDW->throttle_dem;
  }

  STEdot_min = (THRmaxf - STEdot_max) * 0.5;
  STEdot_dem = (THRmaxf - rtDW->throttle_dem) + 0.1;
  STEdot_max = STEdot_dem;
  if (STEdot_dem < -STEdot_min) {
    STEdot_max = -STEdot_min;
  }
  ...
  TASmax = (THRminf - rtDW->throttle_dem) - 0.1;
  STEdot_dem = TASmax;
  if (TASmax < -STEdot_min) {
    STEdot_dem = -STEdot_min;
  }
  ...
  rtDW->integTHR_state += SPE_err_max * rtDW->integGain * rtDW->dt * SPE_err_min;
  STEdot_min = rtDW->integTHR_state;
  if (rtDW->integTHR_state < STEdot_dem) {
    rtDW->integTHR_state = STEdot_dem;
  }

  if (STEdot_min > STEdot_max) {
    rtDW->integTHR_state = STEdot_max;
  }
  ...
  TASmax = (2.0 - THRmaxf) * SPEdot_dem - SKEdot_dem * THRmaxf;
  SPEdot_dem = SPE_dem;
  if (SPE_dem < -SPE_err_max_tmp) {
    SPEdot_dem = -SPE_err_max_tmp;
  }
  ...
  velRateMax = (2.0 - THRmaxf) * SPEdot_dem - (velRateMax - SKE_est) * THRmaxf;
  TASmin = velRateMax * rtDW->integGain;
  if (rtDW->pitch_dem > PITCHmaxf) {
    TASmin = fmin(TASmin, PITCHmaxf - rtDW->pitch_dem);
  } else {
    if (rtDW->pitch_dem < PITCHminf) {
      TASmin = fmax(TASmin, PITCHminf - rtDW->pitch_dem);
    }
  }

  THRminf = TASmin * rtDW->dt;
  TASmin = rtDW->TAS_state * rtDW->timeConstant * rtDW->GRAVITY_MSS;
  TASmax = (TASmax - ((2.0 - THRmaxf) * velRateMin - TAS_dem * THRmaxf)) *
    rtDW->ptchDamp + (TASmax * rtDW->timeConstant + velRateMax);
  velRateMin = (PITCHminf - 0.0783) * TASmin - TASmax;
  TAS_dem = (PITCHmaxf + 0.0783) * TASmin - TASmax;
  SPE_dem = TAS_dem - velRateMin;
  THRmaxf = THRminf;
  velRateMax = -SPE_dem * 0.1;
  SPE_dem *= 0.1;
  if (THRminf < velRateMax) {
    THRminf = velRateMax;
  }
  ...
  velRateMin = fmin(velRateMin, rtDW->integSEB_state);
  TAS_dem = fmax(TAS_dem, rtDW->integSEB_state);
  THRmaxf = rtDW->integSEB_state + THRminf;
  rtDW->integSEB_state = THRmaxf;
  if (THRmaxf < velRateMin) {
    rtDW->integSEB_state = velRateMin;
  }

  if (THRmaxf > TAS_dem) {
    rtDW->integSEB_state = TAS_dem;
  }

  TASmax = (TASmax + rtDW->integSEB_state) / TASmin;
  rtDW->pitch_dem = TASmax;
  if (TASmax < PITCHminf) {
    rtDW->pitch_dem = PITCHminf;
  }

  if (TASmax > PITCHmaxf) {
    rtDW->pitch_dem = PITCHmaxf;
  }

  TASmax = rtDW->dt * rtDW->vertAccLim / rtDW->TAS_state;
  SPEdot_dem = rtDW->pitch_dem - rtDW->last_pitch_dem;
  if (SPEdot_dem > TASmax) {
    rtDW->pitch_dem = rtDW->last_pitch_dem + TASmax;
  } else {
    if (SPEdot_dem < -TASmax) {
      rtDW->pitch_dem = rtDW->last_pitch_dem - TASmax;
    }
  ...
  rtDW)
{
  ...
  if (fabs(Nu) > 2.8274333882308138) {
    ...
    if (fabs(wrap_PI(*target_bearing_cd / rtDW->HD / 100.0 - yawo)) > 12000.0 /
        rtDW->HD / 100.0) {
  ...
  if ((fabs(Nu) > 2.8274333882308138) && (fabs(rtDW->last_Nu) >
    ...
    if ((fabs(wrap_PI(*target_bearing_cd / rtDW->HD / 100.0 - yawo)) > 12000.0 /
         rtDW->HD / 100.0) && (Nu * rtDW->last_Nu < 0.0)) {
      Nuo = rtDW->last_Nu;
    }
  }
  ...
  real_T b_dist_min, DW *rtDW)
{
  ...
  real_T A_air_length;
  real_T L1_dist;
  real_T crosstrack_error;
  real_T target_bearing_cd;
  real_T A_air_0;
  real_T AB_0;
  target_bearing_cd = get_bearing_to(b_next_WP, rtDW->current_loc, rtDW);
  groundSpeed = norm(rtDW->groundspeed_vector);
  if (groundSpeed < 0.1) {
    groundSpeed = 0.1;
    L1_dist = rtDW->yaw;
    if (rtDW->reverse != 0.0) {
      L1_dist = wrap_PI(rtDW->yaw + 3.1415926535897931);
    }
    ...
    rtDW->groundspeed_vector[0] = cos(L1_dist) * 0.1;
    rtDW->groundspeed_vector[1] = sin(A_air_length) * 0.1;
  }

  L1_dist = fmax(0.3183099 * rtDW->L1_damping * rtDW->L1_period * groundSpeed,
                 b_dist_min);
  ...
  if (AB_length < 1.0E-6) {
    ...
    if (AB_length < 1.0E-6) {
      ...
      crosstrack_error = rtDW->yaw;
      if (rtDW->reverse != 0.0) {
        crosstrack_error = wrap_PI(rtDW->yaw + 3.1415926535897931);
      }

      AB[0] = cos(AB_length);
      AB[1] = sin(crosstrack_error);
      AB_length = norm(AB);
    }
  }
  ...
  crosstrack_error = A_air[0] * AB_0 - A_air[1] * AB[0];
  A_air_0 = A_air[0] * AB[0] + A_air[1] * AB_0;
  if ((A_air_length > L1_dist) && (A_air_0 / fmax(A_air_length, 1.0) < -0.7071))
  {
    rtDW->mode_L1 = 0.0;
  ...
  switch ((int32_T)rtDW->mode_L1) {
   case 0:
    ...
    AB_length = -A_air_0;
    crosstrack_error = rtDW->groundspeed_vector[0] * -A_air_0;
    A_air_0 = A_air[1] / A_air_length;
    crosstrack_error += rtDW->groundspeed_vector[1] * -A_air_0;
    A_air_length = rt_atan2d_snf(rtDW->groundspeed_vector[0] * -A_air_0 -
      rtDW->groundspeed_vector[1] * AB_length, crosstrack_error);
    break;
   ...
   case 1:
    ...
    AB_0 = AB[0] / A_air_length;
    AB_length = -AB_0;
    crosstrack_error = rtDW->groundspeed_vector[0] * -AB_0;
    AB_0 = AB[1] / A_air_length;
    crosstrack_error += rtDW->groundspeed_vector[1] * -AB_0;
    A_air_length = rt_atan2d_snf(rtDW->groundspeed_vector[0] * -AB_0 -
      rtDW->groundspeed_vector[1] * AB_length, crosstrack_error);
    break;

   case 2:
    A_air_length = crosstrack_error / fmax(L1_dist, 0.1);
    AB_length = A_air_length;
    ...
    A_air_length = wrap_PI(rt_atan2d_snf(rtDW->groundspeed_vector[0] * AB_0 -
      rtDW->groundspeed_vector[1] * AB[0], rtDW->groundspeed_vector[0] * AB[0] +
      rtDW->groundspeed_vector[1] * AB_0) + A_air_length);
    break;
  ...
  A_air_length = prevent_indecision(A_air_length, &target_bearing_cd, rtDW);
  rtDW->last_Nu = A_air_length;
  target_bearing_cd = A_air_length;
  if (A_air_length < -1.5708) {
    A_air_length = -1.5708;
  }

  if (target_bearing_cd > 1.5708) {
    A_air_length = 1.5708;
  }

  rtDW->latAccDem = 4.0 * rtDW->L1_damping * rtDW->L1_damping * groundSpeed *
    groundSpeed / L1_dist * sin(A_air_length);
}
  ...
  b_loiter_direction, DW *rtDW)
{
  ...
  if ((sanitized_bank_limit == 0.0) || (rtDW->EAS_dem == 0.0) ||
    ...
    b_radius = omega * eas2tas_sq;
  } else {
    sanitized_bank_limit = rtDW->EAS_dem * rtDW->EAS_dem /
      lateral_accel_sea_level;
    if (sanitized_bank_limit > omega) {
      b_radius = omega * eas2tas_sq;
    } else {
      b_radius = fmax(sanitized_bank_limit * eas2tas_sq, omega);
    }
  }

  omega = 6.2832 / rtDW->L1_period;
  eas2tas_sq = norm(rtDW->groundspeed_vector);
  sanitized_bank_limit = fmax(eas2tas_sq, 1.0);
  target_bearing_cd = get_bearing_to(b_center_WP, rtDW->current_loc, rtDW);
  lateral_accel_sea_level = cos(b_center_WP[0] * 1.0E-7 / rtDW->HD);
  if (lateral_accel_sea_level < 0.01) {
    lateral_accel_sea_level = 0.01;
  }

  A_air[0] = (rtDW->current_loc[0] - b_center_WP[0]) *
    rtDW->LOCATION_SCALING_FACTOR;
  A_air[1] = (rtDW->current_loc[1] - b_center_WP[1]) *
    rtDW->LOCATION_SCALING_FACTOR * lateral_accel_sea_level;
  lateral_accel_sea_level = norm(A_air);
  if (lateral_accel_sea_level > 0.1) {
    A_air[0] /= lateral_accel_sea_level;
    A_air[1] /= lateral_accel_sea_level;
  } else if (eas2tas_sq < 0.1) {
    A_air[0] = cos(rtDW->yaw);
    A_air[1] = sin(rtDW->yaw);
  } else {
    A_air[0] = rtDW->groundspeed_vector[0] / eas2tas_sq;
    A_air[1] = rtDW->groundspeed_vector[1] / eas2tas_sq;
  }

  eas2tas_sq = A_air[0] * rtDW->groundspeed_vector[1] - A_air[1] *
    rtDW->groundspeed_vector[0];
  tmp = rtDW->groundspeed_vector[0] * A_air[0] + rtDW->groundspeed_vector[1] *
    A_air[1];
  target_bearing_cd = prevent_indecision(rt_atan2d_snf(eas2tas_sq, -tmp),
    &target_bearing_cd, rtDW);
  rtDW->last_Nu = target_bearing_cd;
  amt = target_bearing_cd;
  if (target_bearing_cd < -1.5707963267948966) {
    target_bearing_cd = -1.5707963267948966;
  }
  ...
  sanitized_bank_limit = 4.0 * rtDW->L1_damping * rtDW->L1_damping *
    sanitized_bank_limit * sanitized_bank_limit / (0.3183099 * rtDW->L1_damping *
    rtDW->L1_period * sanitized_bank_limit) * sin(target_bearing_cd);
  lateral_accel_sea_level -= b_radius;
  omega = 2.0 * rtDW->L1_damping * omega * tmp + omega * omega *
    lateral_accel_sea_level;
  eas2tas_sq *= b_loiter_direction;
  if ((-tmp < 0.0) && (eas2tas_sq < 0.0)) {
    omega = fmax(omega, 0.0);
  }

  omega = (eas2tas_sq * eas2tas_sq / fmax(0.5 * b_radius, b_radius +
            lateral_accel_sea_level) + omega) * b_loiter_direction;
  if ((lateral_accel_sea_level > 0.0) && (b_loiter_direction *
       sanitized_bank_limit < b_loiter_direction * omega)) {
    rtDW->latAccDem = sanitized_bank_limit;
  } else {
    rtDW->latAccDem = omega;
  }
}
...
static void copter_run(DW *rtDW)
{
  ...
  input_euler_angle_roll_pitch_eu(rtDW->roll_target, rtDW->pitch_target,
...
static void auto_mode_4a1(real_T prev_WP[2], real_T next_WP[2], DW *rtDW)
{
  ...
  switch (rtDW->PathModeOut_sl.flightTaskMode) {
   ...
   case HoverUpMode:
    ...
    TAS_dem = rtDW->PathModeOut_sl.heightCmd - rtDW->hgt_dem_cm;
    if (TAS_dem > 8.0) {
      rtDW->climb_rate_cms = rtDW->PathModeOut_sl.maxClimbSpeed;
   ...
   case Rotor2Fix_Mode:
    ...
    if (rtDW->uavMode_j == 1.0) {
      if (rtDW->Rotor2Fix_delay_flag_n != 0.0) {
        ...
        update_speed(rtDW->aerodynamic_load_factor, &TAS_dem, &TASmax, &TASmin,
                     rtDW);
        rtDW->pitch_target = 0.0;
        input_euler_angle_roll_pitch_eu(rtDW->roll_target, 0.0,
      ...
      if (rtDW->aspeed > rtDW->aspeed_c2ps) {
        ...
        TAS_dem = rtDW->p_tilt_pitch_target * -3000.0 + rtDW->pitch_target;
        update_z_controller(rtDW);
        input_euler_angle_roll_pitch_eu(rtDW->roll_target, TAS_dem,
          rtDW->target_yaw_rate, rtDW);
        rate_controller_run(rtDW);
        rtDW->nav_pitch_cd = TAS_dem;
        rtDW->nav_roll_cd = rtDW->roll_target;
        stabilize(rtDW);
   ...
   case PathFollowMode:
    ...
    tmp = (int32_T)rtDW->WP_i_a - 1;
    prev_WP[0] = rtDW->loc.lat[tmp];
    prev_WP[1] = rtDW->loc.lon[tmp];
    tmp = (int32_T)(rtDW->WP_i_a + 1.0) - 1;
    next_WP[0] = rtDW->loc.lat[tmp];
    next_WP[1] = rtDW->loc.lon[tmp];
    get_distance_NE(next_WP, rtDW->current_loc, AB, rtDW);
    ...
    update_waypoint(prev_WP, next_WP, rtDW->dist_min, rtDW);
    plane_run_4a1(rtDW);
    break;
   ...
   case GoHomeMode:
    ...
    prev_WP[0] = rtDW->loc.lat[tmp];
    prev_WP[1] = rtDW->loc.lon[tmp];
    next_WP[0] = rtDW->loc.lat[0];
    next_WP[1] = rtDW->loc.lon[0];
    update_waypoint(prev_WP, next_WP, rtDW->dist_min, rtDW);
    plane_run_4a1(rtDW);
    break;
...
static void auto_mode_sl_4a1(real_T prev_WP[2], real_T next_WP[2], DW *rtDW)
{
  ...
  switch (rtDW->PathModeOut_sl.flightTaskMode) {
    ...
    if (rtDW->uavMode_d == 1.0) {
      ...
      if (rtDW->aspeed > rtDW->aspeed_c2ps) {
        ...
        pitch_target_temp = rtDW->p_tilt_pitch_target * -3000.0 +
          rtDW->pitch_target;
        update_z_controller(rtDW);
        input_euler_angle_roll_pitch_eu(rtDW->roll_target, pitch_target_temp,
          rtDW->target_yaw_rate, rtDW);
        rate_controller_run(rtDW);
   ...
   case PathFollowMode:
    ...
    prev_WP[0] = rtDW->PathModeOut_sl.prePathPoint_LLA[0];
    next_WP[0] = rtDW->PathModeOut_sl.curPathPoint_LLA[0];
    prev_WP[1] = rtDW->PathModeOut_sl.prePathPoint_LLA[1];
    next_WP[1] = rtDW->PathModeOut_sl.curPathPoint_LLA[1];
    update_waypoint(prev_WP, next_WP, rtDW->dist_min, rtDW);
    plane_run_4a1(rtDW);
    break;
   ...
   case GoHomeMode:
    ...
    switch ((int32_T)rtDW->uavMode_d) {
     case 1:
      ...
      prev_WP[0] = rtDW->center_WP[0];
      next_WP[0] = rtDW->PathModeOut_sl.curPathPoint_LLA[0];
      prev_WP[1] = rtDW->center_WP[1];
      next_WP[1] = rtDW->PathModeOut_sl.curPathPoint_LLA[1];
      update_waypoint(prev_WP, next_WP, rtDW->dist_min, rtDW);
      plane_run_4a1(rtDW);
      break;
...
static void run_V10(DW *rtDW)
{
  ...
  switch ((int32_T)rtDW->mode) {
   ...
   case 6:
    ...
    prev_WP[0] = rtDW->loc.lat[prev_WP_tmp];
    prev_WP[1] = rtDW->loc.lon[prev_WP_tmp];
    prev_WP_tmp = (int32_T)(rtDW->WP_i_m + 1.0) - 1;
    next_WP[0] = rtDW->loc.lat[prev_WP_tmp];
    next_WP[1] = rtDW->loc.lon[prev_WP_tmp];
    temp_yaw_rate = cos(rtDW->current_loc[0] * 1.0E-7 / rtDW->HD);
    if (temp_yaw_rate < 0.01) {
      temp_yaw_rate = 0.01;
    }

    next_WP_0[0] = (rtDW->loc.lat[prev_WP_tmp] - rtDW->current_loc[0]) *
      rtDW->LOCATION_SCALING_FACTOR;
    next_WP_0[1] = (rtDW->loc.lon[prev_WP_tmp] - rtDW->current_loc[1]) *
      rtDW->LOCATION_SCALING_FACTOR * temp_yaw_rate;
    if (norm(next_WP_0) < rtDW->L1_radius) {
   ...
   case 9:
    ...
    auto_mode_4a1(prev_WP, next_WP, rtDW);
    break;
   ...
   case 10:
    ...
    auto_mode_sl_4a1(prev_WP, next_WP, rtDW);
    break;
  ...
  switch (rtDW->PathModeOut_sl.flightTaskMode) {
   ...
   case Rotor2Fix_Mode:
    ...
    if (rtDW->uavMode == 1.0) {
      ...
      update_z_controller(rtDW);
      input_euler_angle_roll_pitch_eu(rtDW->roll_target, tail_error,
        rtDW->target_yaw_rate, rtDW);
      rate_controller_run(rtDW);
   ...
   case PathFollowMode:
    ...
    tmp = (int32_T)rtDW->WP_i_o - 1;
    prev_WP[0] = rtDW->loc.lat[tmp];
    prev_WP[1] = rtDW->loc.lon[tmp];
    tmp = (int32_T)(rtDW->WP_i_o + 1.0) - 1;
    next_WP[0] = rtDW->loc.lat[tmp];
    next_WP[1] = rtDW->loc.lon[tmp];
    get_distance_NE(next_WP, rtDW->current_loc, AB, rtDW);
    if (norm(AB) < rtDW->L1_radius) {
...
static void run_V1000(DW *rtDW)
{
  ...
  switch ((int32_T)rtDW->mode) {
   ...
   case 9:
    ...
    auto_mode(prev_WP, next_WP, rtDW);
    break;
   ...
   case 10:
    ...
    auto_mode_sl(prev_WP, next_WP, rtDW);
    break;
...
/* Model step function */
void Copter_Plane_h2g_4_and_1_step(RT_MODEL *const rtM)
{
...
void Copter_Plane_h2g_4_and_1_initialize(RT_MODEL *const rtM)
{
  DW *rtDW = ((DW *) rtM->dwork);
  ...
  /* Start for DataStoreMemory: '<Root>/AC_ATTITUDE_ACCEL_RP_CONTROLLER_MAX_RADSS1' */
  rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLLER = 12.566370614359172;

  /* Start for DataStoreMemory: '<Root>/AC_ATTITUDE_ACCEL_RP_CONTROLLER_MIN_RADSS1' */
  rtDW->AC_ATTITUDE_ACCEL_RP_CONTROLL_j = 0.69813170079773179;

  /* Start for DataStoreMemory: '<Root>/AC_ATTITUDE_ACCEL_Y_CONTROLLER_MAX_RADSS1' */
  rtDW->AC_ATTITUDE_ACCEL_Y_CONTROLLER_ = 2.0943951023931953;

  /* Start for DataStoreMemory: '<Root>/AC_ATTITUDE_ACCEL_Y_CONTROLLER_MIN_RADSS1' */
  rtDW->AC_ATTITUDE_ACCEL_Y_CONTROLLE_g = 0.17453292519943295;

  /* Start for DataStoreMemory: '<Root>/AC_ATTITUDE_CONTROL_ANGLE_LIMIT_THROTTLE_MAX1' */
  rtDW->AC_ATTITUDE_CONTROL_ANGLE_LIMIT = 0.8;

  /* Start for DataStoreMemory: '<Root>/AC_ATTITUDE_THRUST_ERROR_ANGLE1' */
  rtDW->AC_ATTITUDE_THRUST_ERROR_ANGLE = 0.52359877559829882;

  /* Start for DataStoreMemory: '<Root>/AP_MOTORS_MATRIX_YAW_FACTOR_CCW1' */
  rtDW->AP_MOTORS_MATRIX_YAW_FACTOR_CCW = 1.0;

  /* Start for DataStoreMemory: '<Root>/AP_MOTORS_MATRIX_YAW_FACTOR_CW1' */
  rtDW->AP_MOTORS_MATRIX_YAW_FACTOR_CW = -1.0;

  /* Start for DataStoreMemory: '<Root>/ATC_RAT_PIT_FILT1' */
  rtDW->ATC_RAT_PIT_FILT = 20.0;

  /* Start for DataStoreMemory: '<Root>/ATC_RAT_PIT_IMAX1' */
  rtDW->ATC_RAT_PIT_IMAX = 0.1;

  /* Start for DataStoreMemory: '<Root>/ATC_RAT_PIT_I_inint1' */
  rtDW->ATC_RAT_PIT_I_inint = 0.1;

  /* Start for DataStoreMemory: '<Root>/ATC_RAT_PIT_P1' */
  rtDW->ATC_RAT_PIT_P = 0.2;

  /* Start for DataStoreMemory: '<Root>/ATC_RAT_RLL_FILT1' */
  rtDW->ATC_RAT_RLL_FILT = 20.0;

  /* Start for DataStoreMemory: '<Root>/ATC_RAT_RLL_P1' */
  rtDW->ATC_RAT_RLL_P = 0.1;

  /* Start for DataStoreMemory: '<Root>/ATC_RAT_YAW_FILT1' */
  rtDW->ATC_RAT_YAW_FILT = 5.0;

  /* Start for DataStoreMemory: '<Root>/ATC_RAT_YAW_P1' */
  rtDW->ATC_RAT_YAW_P = 0.1;

  /* Start for DataStoreMemory: '<Root>/EAS2TAS1' */
  rtDW->EAS2TAS = 1.0;

  /* Start for DataStoreMemory: '<Root>/EAS_dem1' */
  rtDW->EAS_dem = 17.0;

  /* Start for DataStoreMemory: '<Root>/EAS_dem_cm1' */
  rtDW->EAS_dem_cm = 1700.0;

  /* Start for DataStoreMemory: '<Root>/GRAVITY_MSS1' */
  rtDW->GRAVITY_MSS = 9.80665;

  /* Start for DataStoreMemory: '<Root>/HD1' */
  rtDW->HD = 57.295779513082323;

  /* Start for DataStoreMemory: '<Root>/K_D_yaw1' */
  rtDW->K_D_yaw = 0.5;

  /* Start for DataStoreMemory: '<Root>/K_FF_yaw1' */
  rtDW->K_FF_yaw = 1.0;

  /* Start for DataStoreMemory: '<Root>/K_FF_yaw_inint1' */
  rtDW->K_FF_yaw_inint = 1.0;

  /* Start for DataStoreMemory: '<Root>/K_I_yaw1' */
  rtDW->K_I_yaw = 1.0;

  /* Start for DataStoreMemory: '<Root>/Kx1' */
  rtDW->Kx = 0.2572944297082228;

  /* Start for DataStoreMemory: '<Root>/L1_damping1' */
  rtDW->L1_damping = 0.75;

  /* Start for DataStoreMemory: '<Root>/L1_period1' */
  rtDW->L1_period = 17.0;

  /* Start for DataStoreMemory: '<Root>/L1_radius1' */
  rtDW->L1_radius = 30.0;

  /* Start for DataStoreMemory: '<Root>/LOCATION_SCALING_FACTOR1' */
  rtDW->LOCATION_SCALING_FACTOR = 0.011131884502145034;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACCELERATION_MIN1' */
  rtDW->POSCONTROL_ACCELERATION_MIN = 50.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACCEL_FILTER_HZ1' */
  rtDW->POSCONTROL_ACCEL_FILTER_HZ = 2.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACCEL_XY1' */
  rtDW->POSCONTROL_ACCEL_XY = 100.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACCEL_XY_MAX1' */
  rtDW->POSCONTROL_ACCEL_XY_MAX = 357.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACCEL_Z1' */
  rtDW->POSCONTROL_ACCEL_Z = 250.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACC_Z_D1' */
  rtDW->POSCONTROL_ACC_Z_D = 0.1;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACC_Z_FILT_HZ1' */
  rtDW->POSCONTROL_ACC_Z_FILT_HZ = 20.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACC_Z_FILT_HZ_c2p1' */
  rtDW->POSCONTROL_ACC_Z_FILT_HZ_c2p = 1.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACC_Z_FILT_HZ_inint1' */
  rtDW->POSCONTROL_ACC_Z_FILT_HZ_inint = 20.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACC_Z_IMAX1' */
  rtDW->POSCONTROL_ACC_Z_IMAX = 800.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACC_Z_I_inint1' */
  rtDW->POSCONTROL_ACC_Z_I_inint = 0.1;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_ACC_Z_P1' */
  rtDW->POSCONTROL_ACC_Z_P = 1.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_JERK_RATIO1' */
  rtDW->POSCONTROL_JERK_RATIO = 1.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_LEASH_LENGTH_MIN1' */
  rtDW->POSCONTROL_LEASH_LENGTH_MIN = 100.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_OVERSPEED_GAIN_Z1' */
  rtDW->POSCONTROL_OVERSPEED_GAIN_Z = 2.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_POS_XY_P1' */
  rtDW->POSCONTROL_POS_XY_P = 0.8;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_POS_Z_P1' */
  rtDW->POSCONTROL_POS_Z_P = 3.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_SPEED1' */
  rtDW->POSCONTROL_SPEED = 300.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_SPEED_DOWN1' */
  rtDW->POSCONTROL_SPEED_DOWN = -300.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_SPEED_UP1' */
  rtDW->POSCONTROL_SPEED_UP = 300.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_THROTTLE_CUTOFF_FREQ1' */
  rtDW->POSCONTROL_THROTTLE_CUTOFF_FREQ = 2.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_VEL_ERROR_CUTOFF_FREQ1' */
  rtDW->POSCONTROL_VEL_ERROR_CUTOFF_FRE = 4.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_VEL_XY_FILT_D_HZ1' */
  rtDW->POSCONTROL_VEL_XY_FILT_D_HZ = 5.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_VEL_XY_FILT_HZ1' */
  rtDW->POSCONTROL_VEL_XY_FILT_HZ = 5.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_VEL_XY_I1' */
  rtDW->POSCONTROL_VEL_XY_I = 0.1;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_VEL_XY_IMAX1' */
  rtDW->POSCONTROL_VEL_XY_IMAX = 1000.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_VEL_XY_I_inint1' */
  rtDW->POSCONTROL_VEL_XY_I_inint = 0.1;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_VEL_XY_P1' */
  rtDW->POSCONTROL_VEL_XY_P = 2.0;

  /* Start for DataStoreMemory: '<Root>/POSCONTROL_VEL_Z_P1' */
  rtDW->POSCONTROL_VEL_Z_P = 1.0;

  /* Start for DataStoreMemory: '<Root>/PathModeOut1' */
  rtDW->PathModeOut_sl = Copter_Plane_h2g_4_and_1_rtZBUS_TASK_PATH_OutParam;
  ...
  rtDW->PathModeOut_sl.heightCmd = 10000.0;
  rtDW->PathModeOut_sl.maxClimbSpeed = 100.0;
  rtDW->PathModeOut_sl.curPathPoint_LLA[0] = 4.0E+8;
  rtDW->PathModeOut_sl.prePathPoint_LLA[0] = 4.0E+8;
  rtDW->PathModeOut_sl.curPathPoint_LLA[1] = 1.0001E+9;
  rtDW->PathModeOut_sl.prePathPoint_LLA[1] = 1.0E+9;
  rtDW->PathModeOut_sl.curPathPoint_LLA[2] = 0.0;
  rtDW->PathModeOut_sl.prePathPoint_LLA[2] = 0.0;
  rtDW->PathModeOut_sl.rollCmd = 0.1;
  rtDW->PathModeOut_sl.turnCenterLL[0] = 4.0E+8;
  rtDW->PathModeOut_sl.turnCenterLL[1] = 1.0E+9;

  /* Start for DataStoreMemory: '<Root>/TAS_dem_adj1' */
  rtDW->TAS_dem_adj = 17.0;

  /* Start for DataStoreMemory: '<Root>/TAS_state1' */
  rtDW->TAS_state = 17.0;

  /* Start for DataStoreMemory: '<Root>/accel_pitch_max1' */
  rtDW->accel_pitch_max = 30000.0;

  /* Start for DataStoreMemory: '<Root>/accel_roll_max1' */
  rtDW->accel_roll_max = 72000.0;

  /* Start for DataStoreMemory: '<Root>/accel_xy_angle_max1' */
  rtDW->accel_xy_angle_max = 2000.0;

  /* Start for DataStoreMemory: '<Root>/accel_yaw_max1' */
  rtDW->accel_yaw_max = 18000.0;

  /* Start for DataStoreMemory: '<Root>/aerodynamic_load_factor1' */
  rtDW->aerodynamic_load_factor = 1.0;

  /* Start for DataStoreMemory: '<Root>/air_density_ratio1' */
  rtDW->air_density_ratio = 1.0;

  /* Start for DataStoreMemory: '<Root>/airspeed_max1' */
  rtDW->airspeed_max = 25.0;

  /* Start for DataStoreMemory: '<Root>/airspeed_min1' */
  rtDW->airspeed_min = 13.0;

  /* Start for DataStoreMemory: '<Root>/ang_vel_pitch_max1' */
  rtDW->ang_vel_pitch_max = 360.0;

  /* Start for DataStoreMemory: '<Root>/angle_boost_enabled1' */
  rtDW->angle_boost_enabled = 1.0;

  /* Start for DataStoreMemory: '<Root>/angle_limit_tc1' */
  rtDW->angle_limit_tc = 1.0;

  /* Start for DataStoreMemory: '<Root>/armed1' */
  rtDW->armed = 1.0;

  /* Start for DataStoreMemory: '<Root>/arspeed_filt1' */
  rtDW->arspeed_filt = 35.0;

  /* Start for DataStoreMemory: '<Root>/aspeed1' */
  rtDW->aspeed = 17.0;

  /* Start for DataStoreMemory: '<Root>/aspeed_c2p1' */
  rtDW->aspeed_c2p = 8.0;

  /* Start for DataStoreMemory: '<Root>/aspeed_c2ps1' */
  rtDW->aspeed_c2ps = 13.0;

  /* Start for DataStoreMemory: '<Root>/aspeed_cp1' */
  rtDW->aspeed_cp = 30.0;

  /* Start for DataStoreMemory: '<Root>/attitude_target_quat1' */
  ...
  /* Start for DataStoreMemory: '<Root>/dist_min1' */
  rtDW->dist_min = 50.0;

  /* Start for DataStoreMemory: '<Root>/dt1' */
  rtDW->dt = 0.005;

  /* Start for DataStoreMemory: '<Root>/gains_D_pitch1' */
  rtDW->gains_D_pitch = 0.1;

  /* Start for DataStoreMemory: '<Root>/gains_D_roll1' */
  rtDW->gains_D_roll = 0.05;

  /* Start for DataStoreMemory: '<Root>/gains_I_pitch1' */
  rtDW->gains_I_pitch = 0.1;

  /* Start for DataStoreMemory: '<Root>/gains_I_roll1' */
  rtDW->gains_I_roll = 0.1;

  /* Start for DataStoreMemory: '<Root>/gains_P1' */
  rtDW->gains_P_pitch = 3.0;

  /* Start for DataStoreMemory: '<Root>/gains_P_roll1' */
  rtDW->gains_P_roll = 1.0;

  /* Start for DataStoreMemory: '<Root>/gains_imax_pitch1' */
  rtDW->gains_imax_pitch = 3000.0;

  /* Start for DataStoreMemory: '<Root>/gains_imax_roll1' */
  rtDW->gains_imax_roll = 3000.0;

  /* Start for DataStoreMemory: '<Root>/gains_tau_pitch1' */
  rtDW->gains_tau_pitch = 0.5;

  /* Start for DataStoreMemory: '<Root>/gains_tau_roll1' */
  rtDW->gains_tau_roll = 0.5;

  /* Start for DataStoreMemory: '<Root>/highest_airspeed1' */
  rtDW->highest_airspeed = 35.0;

  /* Start for DataStoreMemory: '<Root>/imax_yaw1' */
  rtDW->imax_yaw = 1500.0;

  /* Start for DataStoreMemory: '<Root>/inint1' */
  rtDW->inint = 1.0;

  /* Start for DataStoreMemory: '<Root>/inint_hgt1' */
  rtDW->inint_hgt = 1.0;

  /* Start for DataStoreMemory: '<Root>/input_tc1' */
  rtDW->input_tc = 0.3;

  /* Start for DataStoreMemory: '<Root>/integGain1' */
  rtDW->integGain = 0.1;

  /* Start for DataStoreMemory: '<Root>/k_throttle_c2p' */
  rtDW->k_throttle_c2p = 0.2;

  /* Start for DataStoreMemory: '<Root>/leash1' */
  rtDW->leash = 100.0;

  /* Start for DataStoreMemory: '<Root>/leash_down_z1' */
  rtDW->leash_down_z = 100.0;

  /* Start for DataStoreMemory: '<Root>/leash_up_z1' */
  rtDW->leash_up_z = 100.0;

  /* Start for DataStoreMemory: '<Root>/loc1' */
  rtDW->loc = rtConstP.loc1_InitialValue;

  /* Start for DataStoreMemory: '<Root>/center_WP1' */
  rtDW->center_WP[0] = 4.0E+8;

  /* Start for DataStoreMemory: '<Root>/current_loc1' */
  rtDW->current_loc[0] = 4.0E+8;

  /* Start for DataStoreMemory: '<Root>/groundspeed_vector1' */
  rtDW->groundspeed_vector[0] = 0.1;

  /* Start for DataStoreMemory: '<Root>/loc_origin1' */
  ...
  /* Start for DataStoreMemory: '<Root>/loiter_direction1' */
  rtDW->loiter_direction = 1.0;

  /* Start for DataStoreMemory: '<Root>/maxClimbRate1' */
  rtDW->maxClimbRate = 5.0;

  /* Start for DataStoreMemory: '<Root>/maxSinkRate1' */
  rtDW->maxSinkRate = 7.0;

  /* Start for DataStoreMemory: '<Root>/minSinkRate1' */
  rtDW->minSinkRate = 3.0;

  /* Start for DataStoreMemory: '<Root>/p_angle_pitch1' */
  rtDW->p_angle_pitch = 3.0;

  /* Start for DataStoreMemory: '<Root>/p_angle_roll1' */
  rtDW->p_angle_roll = 3.0;

  /* Start for DataStoreMemory: '<Root>/p_angle_yaw1' */
  rtDW->p_angle_yaw = 3.0;

  /* Start for DataStoreMemory: '<Root>/p_ff_throttle1' */
  rtDW->p_ff_throttle = 0.35;

  /* Start for DataStoreMemory: '<Root>/p_plane_c2p1' */
  rtDW->p_plane_c2p = 0.1;

  /* Start for DataStoreMemory: '<Root>/p_plane_cp1' */
  rtDW->p_plane_cp = 0.4;

  /* Start for DataStoreMemory: '<Root>/p_tail_tilt1' */
  rtDW->p_tail_tilt = 1.0;

  /* Start for DataStoreMemory: '<Root>/p_tilt_pitch_target1' */
  rtDW->p_tilt_pitch_target = 0.2;

  /* Start for DataStoreMemory: '<Root>/pid_accel_z_reset_filter1' */
  rtDW->pid_accel_z_reset_filter = 1.0;

  /* Start for DataStoreMemory: '<Root>/pid_vel_xy_reset_filter1' */
  rtDW->pid_vel_xy_reset_filter = 1.0;

  /* Start for DataStoreMemory: '<Root>/pitch_limit_max_cd1' */
  rtDW->pitch_limit_max_cd = 2000.0;

  /* Start for DataStoreMemory: '<Root>/pitch_limit_min_cd1' */
  rtDW->pitch_limit_min_cd = -1500.0;

  /* Start for DataStoreMemory: '<Root>/pitch_max_limit1' */
  rtDW->pitch_max_limit = 90.0;

  /* Start for DataStoreMemory: '<Root>/pitch_target_p2c' */
  rtDW->pitch_target_p2c = 500.0;

  /* Start for DataStoreMemory: '<Root>/ptchDamp1' */
  rtDW->ptchDamp = 0.7;

  /* Start for DataStoreMemory: '<Root>/pwm_max1' */
  rtDW->pwm_max = 2000.0;

  /* Start for DataStoreMemory: '<Root>/pwm_min1' */
  rtDW->pwm_min = 1000.0;

  /* Start for DataStoreMemory: '<Root>/pwm_out1' */
  rtDW->pwm_out[0] = 1480.0;
  rtDW->pwm_out[1] = 1480.0;
  rtDW->pwm_out[2] = 1480.0;
  rtDW->pwm_out[3] = 1480.0;

  /* Start for DataStoreMemory: '<Root>/pwm_tail' */
  rtDW->pwm_tail = 1000.0;

  /* Start for DataStoreMemory: '<Root>/radius1' */
  rtDW->radius = 60.0;

  /* Start for DataStoreMemory: '<Root>/rate_bf_ff_enabled1' */
  rtDW->rate_bf_ff_enabled = 1.0;

  /* Start for DataStoreMemory: '<Root>/recalc_leash_xy1' */
  rtDW->recalc_leash_xy = 1.0;

  /* Start for DataStoreMemory: '<Root>/recalc_leash_z1' */
  rtDW->recalc_leash_z = 1.0;

  /* Start for DataStoreMemory: '<Root>/reset_accel_to_lean_xy1' */
  rtDW->reset_accel_to_lean_xy = 1.0;

  /* Start for DataStoreMemory: '<Root>/reset_desired_vel_to_pos1' */
  rtDW->reset_desired_vel_to_pos = 1.0;

  /* Start for DataStoreMemory: '<Root>/reset_rate_to_accel_z1' */
  rtDW->reset_rate_to_accel_z = 1.0;

  /* Start for DataStoreMemory: '<Root>/roll_ff_pitch1' */
  rtDW->roll_ff_pitch = 1.0;

  /* Start for DataStoreMemory: '<Root>/roll_ff_pitch_inint1' */
  rtDW->roll_ff_pitch_inint = 1.0;

  /* Start for DataStoreMemory: '<Root>/roll_limit_cd1' */
  rtDW->roll_limit_cd = 2500.0;

  /* Start for DataStoreMemory: '<Root>/roll_limit_cd_inint1' */
  rtDW->roll_limit_cd_inint = 4500.0;

  /* Start for DataStoreMemory: '<Root>/rot_body_to_ned1' */
  memcpy(&rtDW->rot_body_to_ned[0], &rtConstP.rot_body_to_ned1_InitialValue[0],
         9U * sizeof(real_T));

  /* Start for DataStoreMemory: '<Root>/scaling_speed1' */
  rtDW->scaling_speed = 17.0;

  /* Start for DataStoreMemory: '<Root>/spdCompFiltOmega1' */
  rtDW->spdCompFiltOmega = 2.0;

  /* Start for DataStoreMemory: '<Root>/spdWeight1' */
  rtDW->spdWeight = 1.0;

  /* Start for DataStoreMemory: '<Root>/tail_tilt_c2p1' */
  rtDW->tail_tilt_c2p = -2800.0;

  /* Start for DataStoreMemory: '<Root>/tail_tilt_p2c1' */
  rtDW->tail_tilt_p2c = -1300.0;

  /* Start for DataStoreMemory: '<Root>/tail_tilt_rate1' */
  rtDW->tail_tilt_rate = 1500.0;

  /* Start for DataStoreMemory: '<Root>/thrDamp1' */
  rtDW->thrDamp = 0.7;

  /* Start for DataStoreMemory: '<Root>/thr_out_min' */
  rtDW->thr_out_min = 0.2;

  /* Start for DataStoreMemory: '<Root>/throttle_avg_max1' */
  rtDW->throttle_avg_max = 1.0;

  /* Start for DataStoreMemory: '<Root>/throttle_cruise1' */
  rtDW->throttle_cruise = 15.0;

  /* Start for DataStoreMemory: '<Root>/throttle_cutoff_frequency1' */
  rtDW->throttle_cutoff_frequency = 1.0;

  /* Start for DataStoreMemory: '<Root>/throttle_filter1' */
  rtDW->throttle_filter = 0.4816;

  /* Start for DataStoreMemory: '<Root>/throttle_ground' */
  rtDW->throttle_ground = 0.55;

  /* Start for DataStoreMemory: '<Root>/throttle_hover1' */
  rtDW->throttle_hover = 0.48;

  /* Start for DataStoreMemory: '<Root>/throttle_in1' */
  rtDW->throttle_in = 0.4816;

  /* Start for DataStoreMemory: '<Root>/throttle_max1' */
  rtDW->throttle_max = 100.0;

  /* Start for DataStoreMemory: '<Root>/throttle_off_rate' */
  rtDW->throttle_off_rate = 1.0;

  /* Start for DataStoreMemory: '<Root>/throttle_slewrate1' */
  rtDW->throttle_slewrate = 100.0;

  /* Start for DataStoreMemory: '<Root>/throttle_thrust_max1' */
  rtDW->throttle_thrust_max = 1.0;

  /* Start for DataStoreMemory: '<Root>/thrust_slew_time1' */
  rtDW->thrust_slew_time = 0.3;

  /* Start for DataStoreMemory: '<Root>/timeConstant1' */
  rtDW->timeConstant = 5.0;

  /* Start for DataStoreMemory: '<Root>/use_sqrt_controller1' */
  rtDW->use_sqrt_controller = 1.0;

  /* Start for DataStoreMemory: '<Root>/vel_forward_gain1' */
  rtDW->vel_forward_gain = 1.0;

  /* Start for DataStoreMemory: '<Root>/vel_forward_min_pitch1' */
  rtDW->vel_forward_min_pitch = -5.0;

  /* Start for DataStoreMemory: '<Root>/vel_forward_tail_tilt_max1' */
  rtDW->vel_forward_tail_tilt_max = 2000.0;

  /* Start for DataStoreMemory: '<Root>/vertAccLim1' */
  rtDW->vertAccLim = 5.0;

  /* Start for DataStoreMemory: '<Root>/weathervane_gain1' */
  rtDW->weathervane_gain = 0.01;

  /* Start for DataStoreMemory: '<Root>/weathervane_min_roll1' */
  rtDW->weathervane_min_roll = 8.0;

  /* Start for DataStoreMemory: '<Root>/yaw_max_c2p1' */
  rtDW->yaw_max_c2p = 0.3;

  /* Start for DataStoreMemory: '<Root>/yaw_rate_max1' */
